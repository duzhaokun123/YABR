...
import io.github.duzhaokun123.yabr.core.R as AppR
...
object ActivityHijack : BaseModule(), Core {
    ...
    object ActProxyMgr {
        ...
        private fun collectAllActivities(): List<String> {
            val activities = mutableListOf<String>()
            activities.addAll(ModuleActivities.activities)
            for (className in listOf("io.github.duzhaokun123.codegen.FeatureModuleActivities")) {
                runCatching {
                    val cls = Class.forName(className)
                    val instance = cls.getField("INSTANCE").get(null)
                    @Suppress("UNCHECKED_CAST")
                    val list = cls.getMethod("getActivities").invoke(instance) as List<String>
        ...
        fun isModuleProxyActivity(name: String): Boolean {
            return collectAllActivities().find { it == name } != null
        }
